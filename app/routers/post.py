...
def get_posts(db: Session = Depends(get_db), current_user = int = Depends(oauth.get_current_user), limit: int = 10, skip: int = 0, search: Optional[str]=""): #limit: int=10 to grab the first 10 posts only from the database and skip the first posts. in the url just write for instance: {{URL}}posts?limit=2&skip=1&search=beaches%20something // %20 is a space
    #print(limit)
    #posts = db.query(models.Post).filter(models.Post.title.contains(search)).limit(limit).offset(skip).all() 

    #send a query to the database to retrieve all the posts of the 
    #table posts created by the python model Post within models.py file
